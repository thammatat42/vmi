<?php 
    ...
    $stmt = $connect->prepare("SELECT GROUP_ID,CATEGORY_TH,CATEGORY_ENG,IMG,C_STOCK,TOTAL_MAX,TOTAL_MIN,CREATE_BY,CREATE_DATE,UPDATE_BY,UPDATE_DATE,STATUS_CHG FROM (
        SELECT A.*,SUM(B.MAX_STOCK) AS TOTAL_MAX, SUM(B.MIN_STOCK) AS TOTAL_MIN FROM tb_group_master AS A INNER JOIN tb_stock_master AS B ON A.GROUP_ID = B.GROUP_ID 
        WHERE A.CATEGORY_TH like '%".$search."%' AND A.STATUS_CHG = '0' AND B.STATUS_CHG = 0 GROUP BY B.GROUP_ID
    ) GROUP_MASTER");
    $stmt->execute();
                    ...
                    <div class="col-12">
                        <div class="shadow">
                            <div class="card-header border-0 pt-4">
                                ...
                                <button type="submit" class="btn btn-success my-3" data-toggle="modal" data-target="#modal-upload" id="upload_excel">
                                ...
                                <form action="stock.php" class="search-bar">
                                    <input type="search" name="search" pattern=".*\S.*" placeholder="ค้นหาประเภทสินค้า..">
                                    ...
                                    <div class="col-12 col-sm-3">
                                        <a class="check-stock-modal" data-toggle="modal" data-target="#modal-lg" at="<?php echo $row['GROUP_ID']; ?>">
                                            <div class="card">
                                                ...
                                                <?php } elseif($row['C_STOCK'] <= $row['TOTAL_MIN'] && $row['C_STOCK'] <> 0) { ?>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-sm-4 col-md-2">
                                                            <div class="color-palette-set text-center">
                                                                <div class="bg-success color-palette"><span>คลังสินค้าปกติ..</span></div>
                                                            </div>
                                                        </div>
                                                        <div class="col-sm-4 col-md-2">
                                                            <div class="color-palette-set text-center">
                                                                <div class="bg-danger color-palette" style="color: #ffffff!important;"><span>ควรปรับปรุงสต๊อกสินค้า..</span></div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                            ...
                            <!-- Modal Upload -->
                            <form id="upload_form_stock">
                                <div class="modal fade" id="modal-upload">
                                    <div class="modal-dialog modal-md">
                                        <div class="modal-content bg-secondary">
                                            <div class="modal-header">
                                                <h4 class="text-glow" class="modal-title"><i class="fas fa-arrow-circle-up"></i> อัพโหลดไฟล์สต๊อกสินค้า</h4>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close" id="close_modal_1">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <div class="card-body">
                                                <a href="../../assets/template/update_stock_master.xlsx" class="btn btn-secondary" download>
                                                    <i class="fas fa-arrow-circle-down"></i>
                                                    Stock Master
                                                </a>
                                                <br><br>
                                                <div class="col">
                                                    <label style="font-size: 1rem"> อัพโหลดสต๊อกสินค้าเเบบไฟล์ <label style="color: red;">* (.xlsx, .csv UTF-8 เท่านั้น)</label></label>
                                                        <div class="form-group justify-content-between">
                                                            <div class="custom-file">					
                                                                <input style="color: red;" type="file" name="customfile" class="form-group" id="customfile" required>
                                                                <button style="height: 30px;border-radius: 500px;border: 100px;" type="submit" class="btn btn-success" name="upload_stock" id="upload_stock"><i class="fas fa-arrow-circle-up"></i> Upload </button>
                                                            </div>
                                                        </div>				
                                                </div>		
                                            </div>
...
<script>
    $(function() {
        ...
        $('#upload_form_stock').on('submit', function(e) {
            e.preventDefault();
            var formData = new FormData();
            var upload_stock = $('#upload_stock').val();

            formData.append('customfile', $('input[type=file]')[0].files[0]); 
            formData.append('upload_stock', upload_stock);
            $("#overlay").fadeIn(300);
            $.ajax({  
                type: "POST",  
                url: "../../service/upload/upload_stock.php",
                data: formData,
                contentType: false,
                processData: false
            }).done(function(resp) {
                $("#overlay").fadeOut(300);
                Swal.fire({
                    text: 'อัพเดทสต๊อกเรียบร้อยเเล้ว',
                    icon: 'success',
        ...
        $(document).on('click', '#positive', function(){
            var max_stock = parseInt($('#fix_max_stock_id').val());
            var input_number = parseInt($("#input_number").val()); 
            if(input_number < max_stock) {
                var set_value = $('#input_number').val(input_number + 1); 
            } else {
                var set_value = $('#input_number').val(max_stock); 
            }
        });
        ...
        $.ajax({
            ...
            data.response.forEach(function (item, index){
                ...
                if(item.PREVIOUS_STOCK == 0) {
                    item.PREVIOUS_STOCK = `<span class="badge bg-danger" style="font-size: 0.9rem;">${item.PREVIOUS_STOCK}</span>`
                } else {
                    item.PREVIOUS_STOCK = `<span class="badge bg-warning" style="font-size: 0.9rem;">${item.PREVIOUS_STOCK}</span>`
                }
