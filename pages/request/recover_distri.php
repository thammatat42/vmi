<?php 
    ...
    $stmt_check = $connect->prepare("SELECT COUNT(*) AS CHECK_TRACEBACK FROM tb_products WHERE DATE(CURDATE()) >= RECEIVE_DATE AND STATUS_ID = 0 AND STATUS_CHG = 0");
    $stmt_check->execute();
    $result_check = $stmt_check->fetch(PDO::FETCH_OBJ);
    $CHECK_TRACEBACK = $result_check->CHECK_TRACEBACK;

    if($CHECK_TRACEBACK == 0) {
        $disable = "";
    } else {
        $disable = "<a href='special_distribute.php' class='btn btn-success my-3 '><i class='fas fa-arrow-alt-circle-up'></i> ส่งสินค้าย้อนหลัง</a>";
    }
            ...
            var table = $('#receive_logs').DataTable( {
                ...
                "footerCallback": function ( row, data, start, end, display ) {
                    ...
                    $( api.column( 2 ).footer() ).html(
                        `<div class="btn-group" role="group">
                            <button style="margin-left: -30px; margin-right: -20px; border-radius: 10rem;" type="button" class="btn btn-danger btn-lg" id="cancel_all_order">
                                <i class="fas fa-window-close"></i> ยกเลิกทั้งหมด
                            </button>
                        </div>`
                    );
                },
                ...
                initComplete: function () {
                    ...
                    $(document).on('click', '#cancel_all_order', function(){ 
                        const log_alert = "ดำเนินการยกเลิกสินค้าส่งย้อนหลังทั้งหมดหรือไม่??";
                        Swal.fire({
                            title: log_alert,
                            icon: 'warning',
                            showCancelButton: true,
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({  
                                    type: "GET",  
                                    url: "../../service/request/order-cancel-all-special.php"
                                }).done(function(resp) {
                                    $("#overlay").fadeOut(300);
                                    Swal.fire({
                                        text: resp.message,
